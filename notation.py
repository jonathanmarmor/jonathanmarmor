...
class Note(object):
    def __init__(self, yaml_data=False, pitches=[]):
        ...
        if yaml_data:
            self.load(yaml_data)
        if pitches:
            self.raw_pitches = [Pitch(ps) for ps in pitches]
    ...
    @property
    def duration(self):
        if self.raw_duration:
            return DURATION_SPELLINGS[self.raw_duration]
        else:
            return None
...
def main():
    ...
    if args.test or args.test_readme or args.test_notation:
        ...
        p = Piece(args.source)
        p.write(args.target, yaml=False, ly=True, pdf=True, score=True, parts=True, midi=True)
